...
namespace Hex3Mod.Items
{
    ...
    */
    public class TaxManStatement
    {
        ...
        public static void AddTokens(float TaxManStatement_ChanceToInflict, float TaxManStatement_DamagePerTax, float TaxManStatement_BaseGoldPerTax)
        {
            LanguageAPI.Add("H3_" + upperName + "_NAME", "The Tax Man's Statement");
            LanguageAPI.Add("H3_" + upperName + "_PICKUP", "Chance to apply <style=cShrine>Taxation</style> to enemies on hit, stealing their gold and damaging them whenever they use an ability.");
            LanguageAPI.Add("H3_" + upperName + "_DESC", string.Format("Your attacks have a <style=cIsDamage>{0}%</style> chance <style=cStack>(+{0}% per stack)</style> to apply <style=cShrine>Taxation</style>. Taxed enemies are permanently <style=cShrine>affected whenever they use an ability</style>, taking <style=cIsDamage>{1}%</style> of their health as damage <style=cStack>(Halved against bosses)</style> and surrendering <style=cShrine>{2}</style> gold to the player <style=cStack>(Scales over time)</style>.", TaxManStatement_ChanceToInflict, TaxManStatement_DamagePerTax, TaxManStatement_BaseGoldPerTax));
            LanguageAPI.Add("H3_" + upperName + "_LORE", "They say two things in life are certain: Death and taxes." + "\n\nWith this piece, you have both in your hands.");
        }

        private static void AddHooks(ItemDef itemDef, float TaxManStatement_ChanceToInflict, float TaxManStatement_DamagePerTax, float TaxManStatement_BaseGoldPerTax)
        {

        }

        public static void Initiate(float TaxManStatement_ChanceToInflict, float TaxManStatement_DamagePerTax, float TaxManStatement_BaseGoldPerTax)
        {
            ItemAPI.Add(new CustomItem(itemDefinition, CreateDisplayRules()));
            AddTokens(TaxManStatement_ChanceToInflict, TaxManStatement_DamagePerTax, TaxManStatement_BaseGoldPerTax);
            AddHooks(itemDefinition, TaxManStatement_ChanceToInflict, TaxManStatement_DamagePerTax, TaxManStatement_BaseGoldPerTax);
        }
    }
}
